...
const startIndex = lines.findIndex(line => line.includes('// Expense Claims'));

if (startIndex === -1) {
    console.error('Could not find "// Expense Claims" in schema.prisma');
    process.exit(1);
}

const beforeModels = lines.slice(0, startIndex).join('\n');

const NEW_MODELS = `// Expense Claims
// ==========================================
...
const finalSchema = beforeModels + '\n' + NEW_MODELS;
fs.writeFileSync(schemaPath, finalSchema);
console.log('Replaced Expense & Travel Management tables in schema.');
